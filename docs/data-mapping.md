...
- 14종: 카페, 주유, 편의점, 쇼핑, 교통, 통신, 영화/문화, 배달, 구독/OTT,
  해외, 의료, 마트, PX/군마트, 기타. 오타는 `validate:data`의 (d)로 검출.
- 애매한 항목의 선례:
  - 군마트(PX)·PX·GS25 해군마트 → `PX/군마트` (표시 이름 "PX/군마트")
  - 국군콘도·레스토랑·패스트푸드·어학시험 → `기타`
  - 서점 → `영화/문화` (원문 카테고리 유지)
  - GS 팝서비스 → `편의점`
- 혜택 화면의 칩은 **고정 목록이 아니라** `getAvailableCategories(myBenefits)`로
  "내 카드 혜택에 실제로 있는 카테고리"만 뽑아 만든다. 카드를 추가/삭제하면 자동 갱신되고,
  선택했던 칩이 사라지면 화면이 `전체`로 되돌린다.
...
`check:dupes`가 잡아도 아래는 규칙대로 나눈 정상 행이다. 합치지 않는다.
...
| 항목 | 규칙 |
| --- | --- |
| 보기 방식 | `benefits-view-mode` = `'benefit'`(혜택별) / `'card'`(카드별). localStorage 저장 |
| 카드별 보기 그룹 | `groupBenefitsByCard(내카드, 필터된혜택)` — 혜택 0건인 카드는 섹션 자체를 만들지 않음 |
| 혜택별 보기 카드 필터 | 카드 요약 박스 클릭 = 그 카드만 보기 (다시 누르면 해제). 보기 방식 전환 시 해제 |
| 카드별 보기 카드 클릭 | 그 카드 섹션으로 스크롤 (`card-section-{cardId}`) |
| "혜택 N개" | 카드 요약 박스 = 등록 카드 전체 기준(칩 필터 무관), 카드별 섹션 헤더 = 필터 적용 후 행 수 |
